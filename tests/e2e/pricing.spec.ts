...
import { deleteInquiryById, seedInquiryExtraction } from "./support/database";
...
async function createInquiry(page: Page): Promise<string> {
  ...
  if (!match) {
    throw new Error("Could not read the created inquiry identifier from the URL.");
  }
...
test("reviews an inquiry and calculates authoritative pricing", async ({ page }) => {
  ...
  try {
    ...
    await expect(
      page.getByText("Pricing is locked", {
        exact: true,
      }),
    ).toBeVisible();
    ...
    await expect(page.getByText(/Calculated quantity:\s*20 room-nights/i)).toBeVisible();

    await expect(page.getByText(/Calculated quantity:\s*10 rooms/i)).toBeVisible();
    ...
    for (const catalogItemId of selections) {
      await page
        .locator(`input[type="checkbox"][name="catalogItemId"][value="${catalogItemId}"]`)
        .check();
    }

    await expect(page.locator('input[name="occurrences:hotel_room_night"]')).toHaveValue("1");
    ...
    await expect(page.locator('input[name="occurrences:late_checkout_room"]')).toHaveValue("1");

    await calculateButton.click();

    await expect(
      page.getByText(/Pricing calculated deterministically from catalog 2026-08-demo-v1\./),
    ).toBeVisible();
    ...
    const budgetStatus = page.getByTestId("pricing-budget-status");
    ...
    await expect(page.getByTestId("pricing-total")).toHaveCount(0);
  } finally {
    if (inquiryId) {
