...
import { calculateInquiryPricingInputSchema } from "../src/index";
...
describe("calculateInquiryPricingInputSchema", () => {
  it("accepts ids and coerces form occurrence strings", () => {
    const result = calculateInquiryPricingInputSchema.parse({
      inquiryId,
      selections: [
        {
  ...
  it("rejects an empty selection", () => {
    const result = calculateInquiryPricingInputSchema.safeParse({
      inquiryId,
      selections: [],
    });
