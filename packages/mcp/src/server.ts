...
function jsonToolResult(value: unknown): CallToolResult {
  ...
  if (text === undefined) {
    throw new Error("MCP tool returned a non-serializable result.");
  }
...
function failedToolResult(): CallToolResult {
  return {
    content: [
      {
        type: "text",
...
async function executeTool<T>(
  ...
  try {
    ...
    console.error(`[proposal-agent-mcp] ${toolName} failed: ${errorName}`);

    return failedToolResult();
  }
}

export function createProposalMcpServer(dependencies: ProposalMcpToolDependencies): McpServer {
  ...
  server.registerTool(
    "search_products",
    {
      title: "Search authoritative products",
      description:
      ...
      inputSchema: mcpSearchProductsInputSchema,
    },
    async (input) => executeTool("search_products", () => searchProductsTool(dependencies, input)),
  );

  server.registerTool(
    "calculate_pricing",
    {
      title: "Calculate authoritative pricing",
      description:
      ...
      inputSchema: mcpCalculatePricingInputSchema,
    },
    async (input) =>
      executeTool("calculate_pricing", () => calculatePricingTool(dependencies, input)),
  );

  server.registerTool(
    "validate_proposal",
    {
      title: "Validate persisted proposal draft",
      description:
      ...
      inputSchema: mcpValidateProposalInputSchema,
    },
    async (input) =>
      executeTool("validate_proposal", () => validateProposalTool(dependencies, input)),
  );

  server.registerTool(
    "create_draft",
    {
      title: "Create review-ready proposal draft",
      description:
      ...
      inputSchema: mcpCreateDraftInputSchema,
    },
    async (input) => executeTool("create_draft", () => createDraftTool(dependencies, input)),
  );
