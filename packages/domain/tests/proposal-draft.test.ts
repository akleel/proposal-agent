...
describe("createProposalDraft", () => {
  it("creates an immutable-value snapshot", () => {
    const requirements = ["company offsite", "late checkout"];
    ...
    const draft = createProposalDraft({
      id: "77a37479-886a-4a0a-a933-6e065cc5787c",
      inquiryId: "3ac7f2de-7430-47d6-b63f-9c899eafd248",
    ...
    requirements.push("changed later");
    ...
    expect(draft.catalogVersion).toBe("2026-08-demo-v1");

    expect(draft.resolvedInquiry.requirements).toEqual(["company offsite", "late checkout"]);
    ...
    expect(draft.pricing.totalMinor).toBe(3_000_000);
  });

  it("rejects a pricing budget that differs from the resolved inquiry", () => {
  ...
  it("rejects a tampered pricing line total", () => {
    const pricing = createPricing();

    const pricingLine = pricing.lines[0];
  ...
  it("rejects selections that do not match pricing lines", () => {
    expect(() =>
      createProposalDraft({
        id: "77a37479-886a-4a0a-a933-6e065cc5787c",
        inquiryId: "3ac7f2de-7430-47d6-b63f-9c899eafd248",
