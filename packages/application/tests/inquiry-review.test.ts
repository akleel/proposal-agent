...
{
  private review:
    PersistedInquiryReview | null = null;
  ...
  public async findByInquiryId(
    ...
    if (
      !this.review ||
      this.review.inquiryId !== inquiryId
    ) {
      return null;
    }
  ...
  public async saveDecision(
    ...
    const decisions =
      this.review.decisions.filter(
        (current) =>
          current.field !== decision.field,
      );
...
describe("inquiry review persistence use cases", () => {
  it(
    "persists extraction and exposes unresolved issues",
    async () => {
      const inquiryRepository =
        new MemoryInquiryRepository();
      ...
      const inquiry = {
        ...
        rawText:
          "65 people, preferably late checkout.",
        createdAt:
      ...
      expect(
        result?.unresolvedReviewIssues.map(
          (issue) => issue.field,
        ),
      ).toEqual([
        "requirements.0",
  ...
  it(
    "persists an accepted human decision",
    async () => {
      const inquiryRepository =
        new MemoryInquiryRepository();
      ...
      const result =
        await saveInquiryReviewDecision(
          {
            reviewRepository,
            now: () =>
              new Date(
