...
export async function createProposalDraft(
  dependencies: CreateProposalDraftDependencies,
  ...
  const draft = createProposalDraftSnapshot({
    id: dependencies.generateId(),
    inquiryId: input.inquiryId,
    resolvedInquiry: resolved.inquiry,
    selections: input.selections,
    pricing,
    createdAt: dependencies.now(),
  });

  await dependencies.proposalDraftRepository.create(draft);
