...
import {
  createPersistedProposalDraftUseCase,
...
export async function createProposalDraftAction(
  ...
  if (!parsed.success) {
    return errorState(
      "The proposal draft selection is invalid.",
    );
  }
  ...
  try {
    ...
    if (
      ...
      return errorState(
        "Run and save inquiry extraction before creating a proposal draft.",
      );
    }
    ...
    if (
      ...
      return errorState(
        "Complete deterministic human review before creating a proposal draft.",
      );
    }
    ...
    return errorState(
      "The proposal draft could not be persisted. Please try again.",
    );
  }
