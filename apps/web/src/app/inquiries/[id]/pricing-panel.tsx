...
import type { PricingCatalog } from "@proposal-agent/application";
import type {
  CatalogSelection,
  PricingBasis,
  PricingResult,
...
import { calculateInquiryPricingAction } from "./actions";
import { CreateProposalDraftForm } from "./proposal-draft-form";
import { initialPricingActionState } from "./pricing-types";
...
function isRepeatable(basis: PricingBasis): boolean {
  return basis === "per_person" || basis === "per_day";
}

function calculateNightCount(inquiry: ResolvedInquiry | null): number | null {
...
function DerivedQuantityContext({
  ...
  readonly inquiry: ResolvedInquiry | null;
}) {
  if (basis === "per_room_night") {
    const rooms = inquiry?.rooms ?? null;

    const nights = calculateNightCount(inquiry);

    if (rooms !== null && nights !== null) {
      const quantity = rooms * nights;

      return (
        <div className="text-sm text-zinc-600 sm:text-right">
          <p>
            <span className="font-semibold text-zinc-900">{rooms}</span> rooms ×{" "}
            <span className="font-semibold text-zinc-900">{nights}</span> nights
          </p>
    ...
    return (
      <p className="text-sm text-zinc-500 sm:text-right">
        Quantity derives from reviewed rooms and stay dates.
      </p>
    );
  }
  ...
  if (basis === "per_room") {
    ...
    if (rooms !== null) {
      return (
        <div className="text-sm text-zinc-600 sm:text-right">
          <p>
            <span className="font-semibold text-zinc-900">{rooms}</span> room
            {rooms === 1 ? "" : "s"}
          </p>
    ...
    return (
      <p className="text-sm text-zinc-500 sm:text-right">
        Quantity derives from reviewed room count.
      </p>
    );
  }

  if (basis === "flat") {
    return <p className="text-sm font-medium text-zinc-600 sm:text-right">One flat charge</p>;
  }
...
function PricingSubmitButton() {
  ...
  return (
    ...
    >
      {pending ? "Calculating…" : "Calculate pricing"}
    </button>
  );
}

function BudgetSummary({ pricing }: { readonly pricing: PricingResult }) {
  if (pricing.budgetMinor === null || pricing.differenceFromBudgetMinor === null) {
    return (
      <p data-testid="pricing-budget-status" className="text-sm font-medium text-zinc-700">
        No customer budget is available for comparison.
      </p>
    );
  }
  ...
  if (difference > 0) {
    return (
      <p data-testid="pricing-budget-status" className="text-sm font-semibold text-red-700">
  ...
  if (difference < 0) {
    return (
      <p data-testid="pricing-budget-status" className="text-sm font-semibold text-emerald-700">
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3 id="pricing-result-heading" className="mt-2 text-xl font-semibold text-zinc-950">
            Pricing breakdown
          </h3>
        </div>
            ...
            {pricing.lines.map((line) => (
              <tr key={line.catalogItemId} className="border-b border-zinc-200 last:border-0">
                ...
                <td className="py-4 pr-4 text-zinc-700">
                  {formatMoney(line.unitPriceMinor, pricing.currency)}
                </td>

                <td className="py-4 text-right font-semibold text-zinc-950">
                  {formatMoney(line.lineTotalMinor, pricing.currency)}
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div>
          ...
          <p className="mt-2 text-lg font-semibold text-zinc-900">
            {pricing.budgetMinor === null
              ? "Not provided"
              : formatMoney(pricing.budgetMinor, pricing.currency)}
          </p>

          <div className="mt-2">
            <BudgetSummary pricing={pricing} />
          </div>
        </div>
      ...
      <p className="mt-6 border-t border-zinc-200 pt-4 text-xs leading-5 text-zinc-500">
        Quantities and totals were calculated by deterministic domain code from the reviewed inquiry
        and authoritative server-side catalog.
      </p>

      <CreateProposalDraftForm inquiryId={inquiryId} selections={selections} />
    </section>
  );
}

export function PricingPanel({ inquiryId, catalog, resolvedInquiry }: PricingPanelProps) {
  ...
  const enabled = resolvedInquiry !== null;

  const available = enabled && activeItems.length > 0;
    ...
    >
      <div className="max-w-3xl">
        ...
        <p className="mt-3 text-sm leading-6 text-zinc-600">
          Select products explicitly. The browser submits only catalog identifiers and occurrences.
          Authoritative prices, quantities, totals, and budget comparison are calculated again on
          the server.
        </p>
      </div>
      ...
      {!enabled ? (
        <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4">
          <p className="text-sm font-semibold text-amber-950">Pricing is locked</p>

          <p className="mt-1 text-sm leading-6 text-amber-800">
            Complete extraction and any required human review to produce the deterministic
            ResolvedInquiry boundary first.
          </p>
        </div>
      ) : null}
      ...
      <form action={formAction} className="mt-7">
        ...
        <fieldset disabled={!available} className="space-y-4 disabled:opacity-60">
          {activeItems.map((item) => {
            const checkboxId = `catalog-${item.id}`;
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <label htmlFor={checkboxId} className="cursor-pointer">
                      ...
                      <span className="mt-1 block text-sm text-zinc-600">
                        ...
                        {formatPricingBasis(item.pricingBasis)}
                      </span>
                    </label>
                  </div>

                  {isRepeatable(item.pricingBasis) ? (
                    ...
                    <>
                      <input type="hidden" name={`occurrences:${item.id}`} value="1" />

                      <DerivedQuantityContext basis={item.pricingBasis} inquiry={resolvedInquiry} />
                    </>
                  )}
                </div>
              </div>
          ...
          <div className="flex flex-wrap items-center gap-4 pt-2">
            ...
            <p className="text-xs leading-5 text-zinc-500">
              Demo catalog {catalog.version}. Prices are fictional demo configuration, not market
              claims.
            </p>
          </div>
        </fieldset>
      ...
      {state.pricing && state.selections ? (
        <PricingResultView
          inquiryId={inquiryId}
